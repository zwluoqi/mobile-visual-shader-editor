...
#ifndef node_tex_image_api
#define node_tex_image_api
...
void point_texco_clamp(vec3 vin, sampler2D ima,vec4 texel_size, out vec3 vout)
{
  vec2 half_texel_size = 0.5 / vec2(texel_size.xy);
...
void node_tex_image_cubic(vec3 co, sampler2D ima,vec4 ima_texel_size, out vec4 color, out float alpha)
{
  vec2 tex_size = vec2(ima_texel_size.xy);
  ...
  color = safe_color(texture(ima, final_co.xy)) * s0.x * s0.y;
  color += safe_color(texture(ima, final_co.zy)) * s1.x * s0.y;
  color += safe_color(texture(ima, final_co.xw)) * s0.x * s1.y;
  color += safe_color(texture(ima, final_co.zw)) * s1.x * s1.y;
...
void tex_box_sample_cubic(
    vec3 texco, vec3 N, sampler2D ima,vec4 ima_texel_size, out vec4 color1, out vec4 color2, out vec4 color3)
{
  float alpha;
  /* X projection */
  vec2 uv = texco.yz;
  ...
  node_tex_image_cubic(uv.xyy, ima,ima_texel_size, color1, alpha);
  /* Y projection */
  uv = texco.xz;
  if (N.y > 0.0) {
    uv.x = 1.0 - uv.x;
  }
  node_tex_image_cubic(uv.xyy, ima,ima_texel_size, color2, alpha);
  /* Z projection */
  uv = texco.yx;
  if (N.z > 0.0) {
    uv.x = 1.0 - uv.x;
  }
  node_tex_image_cubic(uv.xyy, ima, ima_texel_size,color3, alpha);
}
...
bool node_tex_tile_lookup(inout vec3 co, sampler3D ima, sampler2D map, vec4 map_texel_size)
{
  ...
  if (tile >= map_texel_size.x)
    return false;

  /* Fetch tile information. */
  float tile_layer = texelFetch(map, vec2(tile, 0), 0).x;
  if (tile_layer < 0)
    return false;

  vec4 tile_info = texelFetch(map, vec2(tile, 1), 0);
...
void node_tex_tile_linear(
    vec3 co, sampler3D ima, sampler2D map,vec4 map_texel_size, out vec4 color, out float alpha)
{
  if (node_tex_tile_lookup(co, ima, map,map_texel_size)) {
    color = safe_color(texture3D(ima, co.xyz));
  }
  else {
...
void node_tex_tile_cubic(
    vec3 co, sampler3D ima,vec4 ima_texel_size_0, sampler1DArray map,vec4 map_texel_size, out vec4 color, out float alpha)
{
  if (node_tex_tile_lookup(co, ima, map,map_texel_size)) {
    vec2 tex_size = vec2(ima_texel_size_0.xy);
    ...
    color = safe_color(texelFetch3D(ima, vec3(final_co.xy, co.z), 0.0)) * s0.x * s0.y;
    color += safe_color(texelFetch3D(ima, vec3(final_co.zy, co.z), 0.0)) * s1.x * s0.y;
    color += safe_color(texelFetch3D(ima, vec3(final_co.xw, co.z), 0.0)) * s0.x * s1.y;
    color += safe_color(texelFetch3D(ima, vec3(final_co.zw, co.z), 0.0)) * s1.x * s1.y;
  }
  else {
    color = vec4(1.0, 0.0, 1.0, 1.0);
  }
...
void node_tex_image_linear_simple(float project, vec3 co, sampler2D ima, out vec4 color, out float alpha)
{
  node_tex_image_linear(co,ima,color,alpha);
}

void node_tex_image_cubic_simple(float project, vec3 co,  sampler2D ima, out vec4 color, out float alpha)
{
  vec2 tex_size = textureSize(ima,0);
  vec4 ima_texel_size = vec4(tex_size,1/tex_size);
  node_tex_image_cubic(co,ima,ima_texel_size,color,alpha);
}
