...
#ifndef node_tex_voronoi_api
#define node_tex_voronoi_api

#include "../include/base_h.cginc"

/*
Voronoi纹理节点在输入纹理坐标处计算Worley Noise。
...
void f1_1d_node_tex_voronoi(vec3 coord,
                            float w,
                            float scale,
                            float smoothness,
                            float exponent,
...
void smooth_f1_1d_node_tex_voronoi(vec3 coord,
                                   float w,
                                   float scale,
                                   float smoothness,
                                   float exponent,
...
void f2_1d_node_tex_voronoi(vec3 coord,
                            float w,
                            float scale,
                            float smoothness,
                            float exponent,
...
void distance_to_edge_1d_node_tex_voronoi(vec3 coord,
                                          float w,
                                          float scale,
                                          float smoothness,
                                          float exponent,
...
void n_sphere_radius_1d_node_tex_voronoi(vec3 coord,
                                         float w,
                                         float scale,
                                         float smoothness,
                                         float exponent,
...
void f1_2d_node_tex_voronoi(vec3 coord,
                            float w,
                            float scale,
                            float smoothness,
                            float exponent,
...
void smooth_f1_2d_node_tex_voronoi(vec3 coord,
                                   float w,
                                   float scale,
                                   float smoothness,
                                   float exponent,
...
void f2_2d_node_tex_voronoi(vec3 coord,
                            float w,
                            float scale,
                            float smoothness,
                            float exponent,
...
void distance_to_edge_2d_node_tex_voronoi(vec3 coord,
                                          float w,
                                          float scale,
                                          float smoothness,
                                          float exponent,
                                          ...
                                          out float outRadius)
{
  ...
  minDistance = 8.0;
  for (int j_j = -1; j_j <= 1; j_j++) {
    for (int i_i = -1; i_i <= 1; i_i++) {
      vec2 cellOffset = vec2(i_i, j_j);
      vec2 vectorToPoint = cellOffset + hash_vec2_to_vec2(cellPosition + cellOffset) * randomness -
                           localPosition;
...
void n_sphere_radius_2d_node_tex_voronoi(vec3 coord,
                                         float w,
                                         float scale,
                                         float smoothness,
                                         float exponent,
                                         ...
                                         out float outRadius)
{
  ...
  for (int j_j = -1; j_j <= 1; j_j++) {
    for (int i_i = -1; i_i <= 1; i_i++) {
      ...
      vec2 cellOffset = vec2(i_i, j_j) + closestPointOffset;
      vec2 pointPosition = cellOffset + hash_vec2_to_vec2(cellPosition + cellOffset) * randomness;
      float distanceToPoint = distance(closestPoint, pointPosition);
      if (distanceToPoint < minDistance) {
        minDistance = distanceToPoint;
...
void f1_3d_node_tex_voronoi(vec3 coord,
                            float w,
                            float scale,
                            float smoothness,
                            float exponent,
...
void smooth_f1_3d_node_tex_voronoi(vec3 coord,
                                   float w,
                                   float scale,
                                   float smoothness,
                                   float exponent,
...
void f2_3d_node_tex_voronoi(vec3 coord,
                            float w,
                            float scale,
                            float smoothness,
                            float exponent,
...
void distance_to_edge_3d_node_tex_voronoi(vec3 coord,
                                          float w,
                                          float scale,
                                          float smoothness,
                                          float exponent,
                                          ...
                                          out float outRadius)
{
  ...
  minDistance = 8.0;
  for (int k_k = -1; k_k <= 1; k_k++) {
  for (int j_j = -1; j_j <= 1; j_j++) {
    for (int i_i = -1; i_i <= 1; i_i++) {
        vec3 cellOffset = vec3(i_i, j_j, k_k);
        vec3 vectorToPoint = cellOffset +
                             hash_vec3_to_vec3(cellPosition + cellOffset) * randomness -
                             localPosition;
...
void n_sphere_radius_3d_node_tex_voronoi(vec3 coord,
                                         float w,
                                         float scale,
                                         float smoothness,
                                         float exponent,
                                         ...
                                         out float outRadius)
{
  ...
  for (int j_j = -1; j_j <= 1; j_j++) {
    for (int i_i = -1; i_i <= 1; i_i++) {
        vec3 cellOffset = vec3(i_i, j_j, k_k);
        if (i_i == 0 && j_j == 0 && k_k == 0) {
          continue;
        }
        cellOffset +=  closestPointOffset;
        vec3 pointPosition = cellOffset +
                             hash_vec3_to_vec3(cellPosition + cellOffset) * randomness;
        float distanceToPoint = distance(closestPoint, pointPosition);
        if (distanceToPoint < minDistance) {
...
void f1_4d_node_tex_voronoi(vec3 coord,
                            float w,
                            float scale,
                            float smoothness,
                            float exponent,
...
void smooth_f1_4d_node_tex_voronoi(vec3 coord,
                                   float w,
                                   float scale,
                                   float smoothness,
                                   float exponent,
...
void f2_4d_node_tex_voronoi(vec3 coord,
                            float w,
                            float scale,
                            float smoothness,
                            float exponent,
...
void distance_to_edge_4d_node_tex_voronoi(vec3 coord,
                                          float w,
                                          float scale,
                                          float smoothness,
                                          float exponent,
                                          ...
                                          out float outRadius)
{
  ...
  minDistance = 8.0;
  for (int u_u = -1; u_u <= 1; u_u++) {
    for (int k_k = -1; k_k <= 1; k_k++) {
      for (int j_j = -1; j_j <= 1; j_j++) {
        for (int i_i = -1; i_i <= 1; i_i++) {
          vec4 cellOffset = vec4(i_i, j_j, k_k, u_u);
...
void n_sphere_radius_4d_node_tex_voronoi(vec3 coord,
                                         float w,
                                         float scale,
                                         float smoothness,
                                         float exponent,
                                         ...
                                         out float outRadius)
{
  ...
  for (int u_u = -1; u_u <= 1; u_u++) {
    for (int k_k = -1; k_k <= 1; k_k++) {
      for (int j_j = -1; j_j <= 1; j_j++) {
        for (int i_i = -1; i_i <= 1; i_i++) {
          if (i_i == 0 && j_j == 0 && k_k == 0 && u_u == 0) {
            continue;
          }
          vec4 cellOffset = vec4(i_i, j_j, k_k, u_u) + closestPointOffset;
          vec4 pointPosition = cellOffset +
                               hash_vec4_to_vec4(cellPosition + cellOffset) * randomness;
          float distanceToPoint = distance(closestPoint, pointPosition);
          if (distanceToPoint < minDistance) {
