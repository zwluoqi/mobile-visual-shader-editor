...
//						DBG("gamepad %d, %d", gev->m_gamepad.idx, gev->m_connected);
				...
				case Event::DropFile:
					{
                        ...
                        inputDropFile(drop->m_filePath);
						//DBG("%s", drop->m_filePath.getCPtr() );
					}
					break;
                case Event::SaveFile:
                    {
                        const SaveFileEvent* drop = static_cast<const SaveFileEvent*>(ev);
                        inputSaveFile(drop->m_filePath);
                    }
                    break;
				default:
					break;
    ...
    void postDialogOpenEventToMainThread(const bx::FilePath &_filePath){
        _postDialogOpenEvent(_filePath);
    }

    void postDialogSaveEventToMainThread(const bx::FilePath &_filePath){
        _postDialogSaveEvent(_filePath);
    }
