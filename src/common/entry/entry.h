...
namespace entry
{
    ...
    struct InputDropResult{
        bx::FilePath filePath;
        uint8_t result;
    };

    struct InputSaveResult{
        bx::FilePath filePath;
        uint8_t result;
    };
    ...
    struct InputCustomData{
        InputDropResult dropFilePath[16];//输入文件
        int dropFileCount{0};//输入文件数量
        
        InputSaveResult saveFilePath;
        bool hasSaveFilePath{false};
    };
    ...
    void postDialogOpenEventToMainThread(const bx::FilePath& filePath);
    void postDialogSaveEventToMainThread(const bx::FilePath& filePath);
