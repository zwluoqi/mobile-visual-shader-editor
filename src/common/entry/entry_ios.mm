...
namespace entry
{
	...
	struct Context
	{
		Context(uint32_t _width, uint32_t _height)
		{
			...
			static const char* const argv[] = { "ios" ,[path UTF8String],[path UTF8String]};
            ...
            if (event)
            {
                switch (event->m_type)
                {
                    case Event::DialogOpen:
                    {
                        ...
                        nfdchar_t *outPath;
                        nfdfilteritem_t filterItem[3] = { { "Texture", "public.image" }, { "NodeShader", "com.vsg.nodedata.document" },{"Model","fbx,obj"} };
                        NFD_OpenDialog(&outPath, filterItem, 3, NULL);
...
{
	bgfx::renderFrame();
    const Event* m_ev =  s_ctx->m_requestQueue.poll() ;
    while (s_ctx->processRequestEvent(m_ev))
    {
        s_ctx->m_requestQueue.release(m_ev);
        m_ev =  s_ctx->m_requestQueue.poll() ;
    }
}
...
{
	BX_UNUSED(touches);
    NSArray<UITouch*> *touch = [[event allTouches] allObjects];
	CGPoint touchLocation = [touch[0] locationInView:self];
	touchLocation.x *= self.contentScaleFactor;
	touchLocation.y *= self.contentScaleFactor;
    
//    NSLog(@"touchesBegan tapCount:%lu",(unsigned long)[touch count]);
...
//    }
    MouseButton::Enum button = [touch count]==2?MouseButton::Right:MouseButton::Left;
	s_ctx->m_eventQueue.postMouseEvent(s_defaultWindow, touchLocation.x, touchLocation.y, 0);
	s_ctx->m_eventQueue.postMouseEvent(s_defaultWindow, touchLocation.x, touchLocation.y, 0, button, true);
}
...
//    NSLog(@"touchesEnded tapCount:%lu",(unsigned long)[touch count]);
    MouseButton::Enum button = [touch count]==2?MouseButton::Right:MouseButton::Left;
	s_ctx->m_eventQueue.postMouseEvent(s_defaultWindow, touchLocation.x, touchLocation.y, 0, button, false);
}
...
//    NSLog(@"touchesMoved tapCount:%lu",(unsigned long)[touch count]);
...
//    NSLog(@"touchesCancelled tapCount:%lu",(unsigned long)[touch count]);
    MouseButton::Enum button = [touch count]==2?MouseButton::Right:MouseButton::Left;
	s_ctx->m_eventQueue.postMouseEvent(s_defaultWindow, touchLocation.x, touchLocation.y, 0, button, false);
}
