...
namespace entry
{
	...
	struct DropFileEvent : public Event
	{
		...
		InputDropResult m_filePath;
	};

    //向外推送保存文件
    struct SaveFileEvent : public Event
    {
        ENTRY_IMPLEMENT_EVENT(SaveFileEvent, Event::SaveFile);

        InputSaveResult m_filePath;
    };

    //请求打开文件窗口
    struct DialogOpenEvent : public Event
    {
        ENTRY_IMPLEMENT_EVENT(DialogOpenEvent, Event::DialogOpen);

        bx::FilePath m_filePath;
    };

    //请求保存文件窗口
    struct DialogSaveEvent : public Event
    {
        ENTRY_IMPLEMENT_EVENT(DialogSaveEvent, Event::DialogSave);
	...
	public:
		...
		void postDropFileEvent(WindowHandle _handle, const char* _filePath,uint8_t result)
		{
			DropFileEvent* ev = BX_NEW(getAllocator(), DropFileEvent)(_handle);
            if(_filePath != nullptr){
                ev->m_filePath.filePath.set( _filePath);
            }
            ev->m_filePath.result = result;
			m_queue.push(ev);
		}
        
        void postSaveFileEvent(WindowHandle _handle, const char* _filePath,uint8_t result)
        {
            SaveFileEvent* ev = BX_NEW(getAllocator(), SaveFileEvent)(_handle);
            if(_filePath != nullptr){
                ev->m_filePath.filePath.set(  _filePath);
            }
            ev->m_filePath.result = result;
        ...
        //请求打开文件
        void requestDialogOpenEvent(WindowHandle _handle,const bx::FilePath& _filePath){
            DialogOpenEvent* ev = BX_NEW(getAllocator(), DialogOpenEvent)(_handle);
            ev->m_filePath = _filePath;
            m_queue.push(ev);
        }
        
        //请求保存文件
        void requestDialogSaveEvent(WindowHandle _handle,const bx::FilePath& _filePath){
            DialogSaveEvent* ev = BX_NEW(getAllocator(), DialogSaveEvent)(_handle);
            ev->m_filePath = _filePath;
            m_queue.push(ev);
        }
