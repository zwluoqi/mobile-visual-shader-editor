...
struct Input
{
    ...
    entry::InputCustomData m_customData;
};
...
void inputDropFile(const entry::InputDropResult& filePath){
    if(s_input->m_customData.dropFileCount>=16){
        DBG("can not continue add file " );
        return;
    }
    s_input->m_customData.dropFilePath[s_input->m_customData.dropFileCount++] = filePath;
}

void inputSaveFile(const entry::InputSaveResult& filePath){
    s_input->m_customData.saveFilePath = filePath;
    s_input->m_customData.hasSaveFilePath = true;
}

const entry::InputDropResult* popDropFile(){
    if(s_input->m_customData.dropFileCount>0){
        return &s_input->m_customData.dropFilePath[--s_input->m_customData.dropFileCount];
    }else{
        return nullptr;
    }
}

const entry::InputSaveResult* popSaveFile(){
    if(s_input->m_customData.hasSaveFilePath){
        s_input->m_customData.hasSaveFilePath = false;
        return &s_input->m_customData.saveFilePath;
    }else{
        return nullptr;
    }
}
