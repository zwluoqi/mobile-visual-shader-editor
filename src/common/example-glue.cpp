...
void showExampleDialog(bool* p_open, const char* _errorText)
{
	char temp[1024];
	bx::snprintf(temp, BX_COUNTOF(temp), "App: %s", "VSG" );
	...
	ImGui::Begin(temp,p_open);

	ImGui::TextWrapped("%s", "visual shader graph" );

    bx::StringView url = "https://github.com/zwluoqi/mobile-visual-shader-editor#license-bsd-2-clause";
	if (!url.isEmpty() )
	{
		ImGui::SameLine();
